...
MeanShiftTracker::MeanShiftTracker(void)
{
}
...
void MeanShiftTracker::computeNormalizedColorHist(const Mat& image, const Mat& histWeights, Mat& hist, int N)
{
	...
	cv::calcHist(&hsv, 1, chanels, cv::Mat(), hist_t, 3, histSize, ranges);
	cv::normalize(hist_t, hist, 1.0);*/
}
...
int MeanShiftTracker::run( const Mat& currentFrame, const int Nmax )
{
	...
	if ( target_initial.area() <= 1 || 
		target_current.area() <= 1 || 
		initialFrame.empty() || 
		currentFrame.empty()
		)
	{
